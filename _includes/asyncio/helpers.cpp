...
struct RecvHandler {
    ...
    void operator()(int res) {
        ...
        // We copy this out because we std::move(m_data) below in the same
        // expression and I'm suspicious about the lifetime of m_data here.
        auto& socket = *m_data.socket;

        // We create a new handler whose buffer is progressed by the appropriate
        // amount.
        m_data.context->async_recv(
            socket, m_buf, m_len, RecvHandler{std::move(m_data), m_buf, m_len});
    }
...
struct SendHandler {
    ...
    void operator()(int res) {
        ...
        // Same as above
        m_data.context->async_send(
            socket, m_buf, m_len, SendHandler{std::move(m_data), m_buf, m_len});
    }
...
void async_send_all(IOContext& context, Socket& socket, const char* buf,
                    size_t len, IOContext::IntFn fn) {
    HandlerData data{&context, &socket, len, std::move(fn)};
